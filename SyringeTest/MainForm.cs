...
namespace SyringeTest
{
    public partial class MainForm : Form
    {
            ...
            if (m_seqMng != null)
            {
                ...
                if (addSeq.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    switch (addSeq.SEQEUNCE_List.seq)
                    {
                        ...
                        case SequenceManager.SEQUENCE.VisionSnap:
                            m_seqMng.AddSeqVisionSnap();
                            item = new ListViewItem(SequenceManager.SEQUENCE.VisionSnap.ToString());
                            item.SubItems.Add("Vision snap");
                            break;
                    }
        ...
        RTSP_Vision m_rtspCamControl = null;
        private void rTSPToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (m_rtspCamControl == null)
            {
                m_rtspCamControl = new RTSP_Vision();
            }

            m_rtspCamControl.Show();
            m_rtspCamControl.Focus();
        }
    }
}
